...
public final class Board implements Cloneable {
	...
	public Board(Board aBoard) {
		field = new byte[aBoard.getWidth()][aBoard.getHeight()];
		for (int i = 0; i < field.length; ++i)
			System.arraycopy(aBoard.field[i], 0, field[i], 0, field[0].length);
		left = aBoard.left;
	}
	...
	public boolean equals(Object obj) {
		...
		for (int x = 0; x < getWidth(); ++x)
			for (int y = 0; y < getHeight(); ++y)
				if (field[x][y] != other.field[x][y]) return false;
		return true;
	}
	...
	public ClearTileResult clearTiles(final Coord... coords) {
		...
		if (stack.isEmpty()) return new ClearTileResult(this, minX, minX, maxX, maxY, false);
